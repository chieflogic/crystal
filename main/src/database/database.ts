...
export class DatabaseService {
  ...
  createFolder(
    ...
    `
      ...
      .get(parentFolderId ? [projectId, parentFolderId] : projectId) as {
  ...
  getLastAppOpen(): {
    ...
    `
      ...
      .get() as
      | { opened_at: string; welcome_hidden: number; discord_shown: number }
      | undefined;
    ...
    if (updates.state !== undefined) {
      ...
      if (existingPanel) {
        console.log(
          ...
          JSON.stringify(existingPanel.state)
        );
      }
    }
      ...
      if (setClauses.length > 0) {
        ...
        if (updates.state !== undefined && result.changes > 0) {
          ...
          console.log(
            ...
            JSON.stringify(afterPanel?.state)
          );
        }
      }
    });
