...
export const PanelTabBar: React.FC<PanelTabBarProps> = memo(
  ({
    ...
    const handlePanelClose = useCallback(
      (e: React.MouseEvent, panel: ToolPanel) => {
        e.stopPropagation();
        ...
        if (panel.type === 'logs') {
          ...
          if (logsState?.isRunning) {
            alert(
              'Cannot close logs panel while process is running. Please stop the process first.',
            );
            return;
          }
        }
    ...
    const availablePanelTypes = (
      Object.keys(PANEL_CAPABILITIES) as ToolPanelType[]
    ).filter((type) => {
      const capabilities = PANEL_CAPABILITIES[type];

      // Filter based on context
      if (context === 'project' && !capabilities.canAppearInProjects)
        return false;
      if (context === 'worktree' && !capabilities.canAppearInWorktrees)
        return false;
      ...
      <div className="panel-tab-bar bg-gray-800 border-b border-gray-700">
        ...
        <div className="flex flex-wrap items-center min-h-[2rem]">
          ...
          {panels.map((panel) => {
            ...
            // Check if setup tasks are complete (allowing close)
            let canClose = !isPermanent;
            ...
            // Determine tooltip based on panel type and status
            let tooltipText: string | undefined;
            if (isPermanent && !canClose) {
              if (panel.type === 'setup-tasks') {
                tooltipText = 'Complete all setup tasks to dismiss this panel';
              } else {
                tooltipText = 'This panel cannot be closed';
              }
            }
              ...
              >
                ...
                {isEditing ? (
                  <input
                    ...
                    style={{
                      width: `${Math.max(50, editingTitle.length * 8)}px`,
                    }}
                  />
                ) : (
                  <>
