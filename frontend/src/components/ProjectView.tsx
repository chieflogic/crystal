...
export const ProjectView: React.FC<ProjectViewProps> = ({
  ...
  const [mainRepoSessionId, setMainRepoSessionId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    if (mainRepoSessionId) {
      console.log(
        '[ProjectView] Loading panels for project session:',
        mainRepoSessionId,
      );
      panelApi
        .loadPanelsForSession(mainRepoSessionId)
        .then(async (loadedPanels) => {
          console.log('[ProjectView] Loaded panels:', loadedPanels);

          // Check if dashboard panel exists, create if not
          const dashboardPanel = loadedPanels.find(
            (p) => p.type === 'dashboard',
          );
          const setupTasksPanel = loadedPanels.find(
            (p) => p.type === 'setup-tasks',
          );
  ...
  const handlePanelClose = useCallback(
    async (panel: ToolPanel) => {
      ...
      // Don't allow closing dashboard panel (it's the permanent home)
      if (panel.type === 'dashboard') {
        console.log('[ProjectView] Cannot close permanent panel:', panel.type);
        return;
      }

      // Setup tasks panel can only be closed if all tasks are complete
      if (panel.type === 'setup-tasks') {
        const setupState = panel.state?.customState as
          | { tasksCompleted?: Record<string, boolean> }
          | undefined;
        const tasksCompleted = setupState?.tasksCompleted || {};
        const allComplete = Object.values(tasksCompleted).every(
          (status) => status,
        );

        if (!allComplete) {
          console.log(
            '[ProjectView] Cannot close setup-tasks panel - tasks not complete',
          );
          alert(
            'Please complete all setup tasks before dismissing this panel.',
          );
          return;
        }
      }
      ...
      let nextPanel =
        sessionPanels[panelIndex + 1] || sessionPanels[panelIndex - 1];

      // If no other panel or the next panel is the same, fall back to dashboard
      if (!nextPanel || nextPanel.id === panel.id) {
        nextPanel =
          sessionPanels.find((p) => p.type === 'dashboard') || sessionPanels[0];
      }
  ...
  useEffect(() => {
    ...
    const getMainRepoSession = async () => {
      ...
      try {
        const response =
          await API.sessions.getOrCreateMainRepoSession(projectId);
  ...
  useEffect(() => {
    ...
    let previousSession = useSessionStore
      .getState()
      .sessions.find((s) => s.id === mainRepoSessionId);
  ...
  useEffect(() => {
    ...
    const unsubscribeCreated =
      window.electronAPI?.events?.onPanelCreated?.(handlePanelCreated);
            ...
            >
              {sessionPanels.map((panel) => (
                <div
                  ...
                  style={{
                    display:
                      panel.id === currentActivePanel.id ? 'block' : 'none',
                  }}
                >
                  <PanelContainer
                    panel={panel}
